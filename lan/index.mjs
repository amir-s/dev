  ...
  if (command === "scan") {
    ...
    if (outputList || skipMacLookup) {
      ...
      for (let i = 0; i < table.length; i++) {
        ...
        console.log(
          `⚡ ${ip.padEnd(15, " ").white} ${`(${mac})`.gray} ${vendor.padEnd(maxWith, " ").gray
          } ${domain.yellow}`
        );
      }
    } else {
      ...
      for (const vendor in groups) {
        ...
        for (const { ip, mac, domain } of group) {
          ...
          console.log(
            ` ⚡ ${ip.padEnd(15, " ").white} ${`(${mac})`.gray} ${domain.green} ${serviceDomains.yellow
            }${services.blue}`
          );
        }
        console.log();
    ...
    const [source, target] = args.slice(1).map(p => p.includes('@') ? p : path.resolve(p));
    if (!source || !target) {
    ...
    let syncing = false;
    const sync = async () => {
      return await $`rsync --exclude ${ignoreDotFiles ? "**/.*" : "_nonex_"} --exclude ${ignoreNodeModules ? "**/node_modules/**" : "_nonex_"} -ah ${source} ${target}`;
    };
